...
⏱️ Animation Orbitale en Temps Réel — Un moteur de simulation orbitale mathématique anime chaque satellite en temps réel en fonction de son mouvement moyen réel (révolutions par jour) et de son inclinaison. Le globe 3D a été fixé (caméra statique) pour permettre une observation pure et scientifique du trafic orbital lorsque le mode "Play" est activé.

📚 Enrichissement Wikipedia & Smart OSINT Fallback — Un clic sur n'importe quel satellite déclenche une recherche dynamique via l'API REST de Wikimedia. Si l'objet est célèbre, son histoire s'affiche. S'il s'agit d'un objet classifié ou d'un débris obscur (Wikipedia 404), le système génère automatiquement une Fiche Technique de Télémétrie listant l'altitude, l'inclinaison, le propriétaire et le statut IA en temps réel.
...
The application ingests live Two-Line Element (TLE) data and the full SATCAT catalogue published by NORAD via CelesTrak, covering more than 17,000 space objects, including active payloads and massive debris swarms. Every object is rendered in real time on an interactive WebGL-powered 3D globe, allowing analysts to visually explore the near-Earth space domain.
...
🛰️ Massive 3D Visualisation & Debris Swarms — Renders 17,000+ tracked space objects simultaneously. The platform actively tracks not only Payloads but also major historical debris swarms (Iridium-33, Cosmos-2251, Fengyun-1C).
...
⏱️ Real-Time Orbital Animation — A client-side simulation engine animates every satellite based on its true mean motion and inclination. The 3D camera is locked by default to allow pure scientific observation of the orbital traffic patterns.
