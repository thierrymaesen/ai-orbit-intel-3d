...
var myGlobe = Globe()(document.getElementById("globeViz"))
    ...
    .pointLat("lat")
    .pointLng("lng")
    .pointAltitude(function(d) { return visualAltitude(d.altitude); })
    ...
    .pathColor(function(d) {
               if (d.norad_id === selectedSatelliteId) return "rgba(255,0,0,0.6)";
               if (d.is_anomaly) return "rgba(242,198,65,0.4)";
               return "rgba(4,66,191,0.2)";
    })
    .pathStroke(2)
...
// -------------------------------------------------------------------
// Update Vectors (Paths)
// -------------------------------------------------------------------
function updateVectors() {
       if (showVectors && currentData.length > 0) {
                  var paths = currentData.map(function(d) {
                                 return {
                                                    path: [
                                                                           [d.lat, d.lng, 0.0],
...
async function fetchWikipediaInfo(satelliteName) {
    ...
    try {
               ...
               elWikiContent.textContent = data.extract || "No historical data found for this object in Wikipedia.";
    } catch (err) {
...
async function fetchData() {
       try {
           ...
           // Store globally for selection refresh & vectors
           currentData = points;

           myGlobe.pointsData(points);
                  myGlobe.ringsData(rings);

           // Update vectors if active
           updateVectors();
