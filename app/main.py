...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        logger.info("[3/3] Training Isolation Forest...")
        ...
        logger.info("READY: %d satellites, %d anomalies.", len(sats), n_anomalies)
...
app = FastAPI(
    ...
    description="Real-time orbital anomaly detection API. Sprint 7.",
    version="0.7.0",
    lifespan=lifespan,
)
...
async def get_positions(
    filter_type: str = Query(default="ALL", description="ALL, LEO, MEO, GEO, ANOMALIES, or TOP10."),
) -> PositionsResponse:
    ...
    for sat in satellites:
        ...
        positions.append(SatellitePosition(
            name=sat.name, norad_id=norad_id,
            lat=round(lat, 4), lon=round(lon, 4), alt=round(alt, 2),
            orbit_type=orbit_type, anomaly_score=round(score, 4), is_anomaly=flagged,
        ))
...
async def ingest_tle(
    ...
    try:
        ...
        return IngestResponse(status="ok", file_path=str(result_path), satellite_count=len(satellites))
    except RuntimeError as exc:
        raise HTTPException(status_code=502, detail=f"TLE download failed: {exc}") from exc
...
async def analyse(
    ...
    try:
        ...
        if df.empty:
            raise HTTPException(status_code=422, detail="No satellites could be parsed.")
        ...
        for norad_id, row in df_result.iterrows():
            records.append(SatelliteAnomaly(
                name=row["name"], norad_id=int(norad_id),
                inclination=round(float(row["inclination"]), 6),
                eccentricity=round(float(row["eccentricity"]), 6),
                mean_motion=round(float(row["mean_motion"]), 6),
                bstar=float(row["bstar"]),
        ...
        report = AnomalyReport(
            ...
            contamination_rate=contamination, satellites=records,
        )
        _state["last_report"] = report
        return report
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail="TLE data not found.") from exc
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc


@app.get("/api/v1/anomalies", response_model=List[SatelliteAnomaly], tags=["results"])
async def get_anomalies(
    ...
    results = sorted(report.satellites, key=lambda s: s.anomaly_score, reverse=True)
    if min_score is not None:
        results = [s for s in results if s.anomaly_score >= min_score]
    return results[:top_n]


@app.get("/api/v1/satellite/{norad_id}", response_model=SatelliteAnomaly, tags=["results"])
